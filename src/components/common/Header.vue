<template>
    <header class="bg-[#161616] md:sticky shadow-2xl top-0 z-50 ">
                ...
                <div
                    ...
                    <ul class="flex md:flex-row flex-col md:items-center gap-8">
                        <li v-for="(item, i) in navLink" :key="i">
                            <nuxt-link :to="item.link" class="font-['Young_Serif' 'serif'] font-[600] text-white hover:text-[#416bfb] text-[20px]">{{ item.level
                            }}</nuxt-link>
                        </li>
                    </ul>
                </div>
                <div class="flex items-center gap-6">
                    <button @click="downloadCv" class="font-['Young_Serif' 'serif'] font-[600] bg-gradient-to-r from-sky-500 to-indigo-500 text-white px-5 py-2 rounded-full">Download CV</button>
...
const downloadCv = () => {
    ...
    link.href = require;
    link.setAttribute('download', 'Anik_Chandra_Resume.pdf');
